...
class GetFTXPriceData:
    ...
    """
    def __init__(self, interval: str, date_start: str, date_stop: str, ticker: str = 'LUNC/USD'):
        self.interval, self.ticker = interval, ticker
        self.timestamp_start, self.timestamp_end = self.get_timestamp_for_date(date_start), self.get_timestamp_for_date(date_stop)
    ...
    def get_candlestick_data(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame(requests.get(
                self.get_url_with_params(self.timestamp_start,
                                         self.offset_timestamp(self.timestamp_start, 1))).json()['result'])
    ...
    def get_data_from_timestamps(self, df) -> pd.DataFrame:
        ...
        for start, end in self.get_timestamps_list():
            df = df.append(
                pd.DataFrame(requests.get(self.get_url_with_params(start, end)).json()['result']), ignore_index=True) \
                if end < self.timestamp_end else\
                df.append(
                    pd.DataFrame(requests.get(self.get_url_with_params(start, self.timestamp_end)).json()['result']),
                ignore_index=True)
        return df
    ...
    def get_timestamps_list(self) -> list:
        ...
        """
        return [[self.offset_timestamp(self.timestamp_start, i+1), self.offset_timestamp(self.timestamp_end, i+2)-1]
    ...
    def get_api_calls_number(self) -> int:
        ...
        """
        return math.ceil((self.timestamp_end - self.timestamp_start)/(self.convert_interval()*1500))
    ...
    def convert_interval(self) -> int:
        ...
        if 'd' in interval:
            if int(num) in range(1, 31):
                ...
                raise Exception('Unsupported interval. For days, your interval should be an int the range of 1 to 30.')
        elif 'h' in interval:
            if int(num) in (1, 6):
                return int(num) * 3600
            else:
                raise Exception('Unsupported interval. For hours, your interval should be either 1hrs or 6hrs')
        elif 'm' in interval:
            if int(num) in (1, 5, 15):
                return int(num) * 60
            else:
                raise Exception('Unsupported interval. For minutes, your interval should be either 1min, 5 mins or 15mins')
        else:
            raise Exception('Unsupported interval. Try d for days, h for hours or m for minutes')
